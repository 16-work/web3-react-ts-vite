...
/** 默认语言 */
const finishLanguage = 'en'; // 系统语言不在支持列表内时，默认的语言

/* 方案1：默认指定语言 */
// export const LANGUAGE = localCache.get('language', finishLanguage);

/* 方案2: 默认跟随浏览器语言（支持语言自动转换） */
const systemLanguage = localCache.get('language', navigator.language); // 系统语言
export const LANGUAGE = languageType.includes(systemLanguage)
