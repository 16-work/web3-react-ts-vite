...
Object.keys(obj).map((v) => {
  ...
  obj[v].map((item) => {
    ...
    oEn[item[0]] = item[2];
    oTw[item[0]] = traditionalized(item[1]);
  });
  obj[v] = o;
  objEn[v] = oEn;
  objTw[v] = oTw;
...
//console.log(obj)
// fs.writeFile(path.resolve(__dirname, '../language/zh-CN.ts'), `export default` + JSON.stringify(obj), (err) => {
//   if (err) {
//     console.error(err);
//   }
//   console.log('----------新增zh-CN.ts成功-------------');
// });

fs.writeFile(path.resolve(__dirname, '../language/en.ts'), 'export default' + JSON.stringify(objEn), (err) => {
  if (err) {
    console.error(err);
  }
  console.log('----------新增en.ts成功-------------');
});

fs.writeFile(path.resolve(__dirname, '../language/zh-TW.ts'), 'export default' + JSON.stringify(objTw), (err) => {
  if (err) {
    console.error(err);
  }
  console.log('----------新增zh-TW.ts成功-------------');
});
